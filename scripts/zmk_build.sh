...
[[ -z $SHIELDS ]] && SHIELDS="$(yq -r '.include[].shield' $HOST_CONFIG_DIR/build.yaml)"
...
compile_board() {
    ...
    [[ $MULTITHREAD = "true" ]] || echo -en "\n$(tput setaf 2)Building $1 with $2... $(tput sgr0)"
    [[ $MULTITHREAD = "true" ]] && echo -e "$(tput setaf 2)Building $1 with $2... $(tput sgr0)"
    $DOCKER_PREFIX west build -d "build/$BUILD_DIR" -p -b $1 $WEST_OPTS \
        -- -DZMK_CONFIG="$CONFIG_DIR" -DZMK_EXTRA_MODULES="$MODULES" -Wno-dev -DSHIELD="$2" -DCONFIG_BUILD_OUTPUT_UF2=y > "$LOGFILE" 2>&1
    if [[ $? -eq 0 ]]; then
...
if [[ $MULTITHREAD = "true" ]]; then
    ...
    for shield in $SHIELDS; do
        SHIELDS_ARRAY+=("${shield}")
    done
    for index in "${!BOARD_ARRAY[@]}"; do
        board="${BOARD_ARRAY[$index]}"
        shield="${SHIELDS_ARRAY[$index]}"
