...
_parse_targets $expr:
    #!/usr/bin/env bash
    attrs="[.board, .shield, .snippet, .cmakeargs]"
    filter="(($attrs | map(. // [.]) | combinations), ((.include // {})[] | $attrs)) | join(\",\")"
    echo "$(yq -r "$filter" build.yaml | grep -v "^," | grep -i "${expr/#all/.*}")"

# build firmware for single board & shield combination
_build_single $board $shield $snippet $cmakeargs *west_args:
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    west build -s zmk/app -d "$build_dir" -b $board {{ west_args }} ${snippet:+-S "$snippet"} -- \
        -DZMK_CONFIG="{{ config }}" ${shield:+-DSHIELD="$shield"} ${cmakeargs:+$cmakeargs}
...
build expr *west_args: _parse_combos
    ...
    echo "$targets" | while IFS=, read -r board shield snippet cmakeargs; do
        just _build_single "$board" "$shield" "$snippet" "$cmakeargs" {{ west_args }}
    done
...
draw:
    #!/usr/bin/env bash
    set -euo pipefail
    keymap -c "{{ draw }}/config.yaml" parse -z "{{ config }}/cornedong.keymap" --virtual-layers Combos >"{{ draw }}/cornedong.yaml"
    yq -Yi '.combos.[].l = ["Combos"]' "{{ draw }}/cornedong.yaml"
